...
# Modified by JY:
#  * Use cryptographically strong random numbers from OpenSSL for generating secure cookies.
...
def _sessionCookie():
    return ''.join(["%02x" % ord(r) for r in rand.bytes(16)])
...
class GuardSession(components.Componentized):
    ...
    def expire(self):
        ...
        if self.uid in self.guard.sessions:
            log.msg("expired session %s..." % str(self.uid)[0:4])
    ...
    def updateCookie(self, newCookie):
        if self.uid in self.guard.sessions:
            sess = self.guard.sessions[self.uid]
            del self.guard.sessions[self.uid]
            self.uid = newCookie
            self.guard.sessions[newCookie] = sess
...
class SessionWrapper:
    ...
    def renderHTTP(self, ctx):
        try:
            ...
            print "Exception in Nevow/guard/renderHTTP:"
            traceback.print_exc()
            return Forbidden().renderHTTP(ctx)
    ...
    def locateChild(self, ctx, segments):
        try:
            ...
            if path.startswith(SESSION_START):
                # SESSION_START found in URL
                if cookie and cookie in self.sessions:
                    # have cookie
                    self.sessions[cookie].setLifetime(self.sessionLifetime)
                    self.sessions[cookie].sessionJustStarted = True
                    return urlToChild(ctx, *segments[1:]), ()
                else:
                    # no cookie or mismatched cookie
                    return Forbidden(), ()
            else:
                # we may or may not have a session at this point
                return self._delegate(ctx, segments)
        except Exception:
            print "Exception in Nevow/guard/locateChild:"
            traceback.print_exc()
            return Forbidden(), ()
    ...
    def checkLogin(self, ctx, session, segments):
        ...
        if segments and segments[0] in self.avatars:
            authCommand = segments[0]
        else:
            authCommand = None

        if authCommand == AUTH_AVATAR and self.auth_avatar_handler:
            return self.auth_avatar_handler(session, request, segments)
        elif authCommand == LOGIN_AVATAR:
            subSegments = segments[1:]
            def unmangleURL((res,segs)):
                # change session cookie after successful login
                postAuthCookie = self.genCookie(ctx, self.sessionLifetime);
                session.updateCookie(postAuthCookie)
